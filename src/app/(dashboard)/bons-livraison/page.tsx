...
import { Truck, Plus, Search, RefreshCw, AlertCircle } from "lucide-react";
...
function mapDoc(d: Models.Document, contacts: any[]): BLItem {
  const s = d as unknown as Record<string, unknown>;
  const cid = rs(s, "contactId");
  const c = contacts.find(e => e.$id === cid);
  return { id: d.$id, number: rs(s, "number") || `BL-${d.$id.slice(0,6)}`, date: rs(s, "date") || d.$createdAt, status: rs(s, "status") || "DRAFT", totalTTC: rn(s, "totalTTC"), totalHT: rn(s, "totalHT"), totalTVA: rn(s, "totalTVA"), dueDate: rs(s, "dueDate") || null, notes: rs(s, "notes"), footer: rs(s, "footer"), linesJson: rs(s, "linesJson"), contactId: cid, contact: { name: c?.nameOrCompany || c?.name || "Client Inconnu", companyName: c?.category === "COMPANY" ? c.nameOrCompany : "", city: c?.city || "", ice: c?.ice || "" } };
}

export default function BonsLivraisonPage() {
  const { userId } = useAuth();
  const [docs, setDocs] = React.useState<BLItem[]>([]);
  ...
  const load = React.useCallback(async () => {
    ...
    try {
      const [dr, cr] = await Promise.all([
        databases.listDocuments(databaseId, documentsCollectionId, [Query.equal("userId", userId), Query.equal("type", "BON_LIVRAISON"), Query.orderDesc("$createdAt"), Query.limit(100)]),
        databases.listDocuments(databaseId, "contacts", [Query.equal("userId", userId), Query.equal("type", "CLIENT"), Query.limit(100)])
      ]);
      setDocs(dr.documents.map(d => mapDoc(d, cr.documents)));
    } catch { setError("Impossible de charger les bons de livraison."); setDocs([]); }
    ...
    <PageTransition className="flex flex-col gap-6">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          <div className="flex size-10 items-center justify-center rounded-lg bg-primary/10"><Truck className="size-5 text-primary" /></div>
          <div><h1 className="text-lg font-semibold tracking-tight">Bons de Livraison</h1><p className="text-xs text-muted-foreground">{docs.length} bon{docs.length !== 1 ? "s" : ""}</p></div>
        </div>
        <Button className="gap-2" nativeButton={false} render={<Link href="/bons-livraison/new" />}><Plus className="size-4" /><span className="hidden sm:inline">Nouveau BL</span><span className="sm:hidden">Nouveau</span></Button>
      </div>
