...
import { FileCheck, Plus, Search, Filter, RefreshCw, AlertCircle } from "lucide-react";
...
const STATUS_STYLES: Record<string, string> = {
  ...
  ISSUED: "border-amber-300 text-amber-700 bg-amber-50",
  APPLIED: "border-blue-300 text-blue-700 bg-blue-50",
  REFUNDED: "border-green-400 text-green-800 bg-green-50",
...
function mapAvoirDocument(document: Models.Document, contacts: any[]): AvoirListItem {
  ...
  return {
    id: document.$id,
    number: readString(source, "number") || `AV-${document.$id.slice(0, 6)}`,
...
export default function AvoirsPage() {
  ...
  const [documents, setDocuments] = React.useState<AvoirListItem[]>([]);
  const [search, setSearch] = React.useState("");
  const [isLoading, setIsLoading] = React.useState(true);
  const [error, setError] = React.useState<string | null>(null);

  const loadAvoirs = React.useCallback(async () => {
    if (!userId) return;
    ...
    try {
      ...
      setDocuments(docsResp.documents.map(doc => mapAvoirDocument(doc, contactsResp.documents)));
    } catch {
      setError("Impossible de charger les avoirs depuis Appwrite.");
      setDocuments([]);
    } finally {
      setIsLoading(false);
    }
    ...
    <PageTransition className="flex flex-col gap-6">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-lg font-semibold tracking-tight">Avoirs</h1>
            <p className="text-xs text-muted-foreground">
              Notes de crédit et remboursements client
            </p>
          </div>
        </div>
        <Button className="gap-2" nativeButton={false} render={<Link href="/avoirs/new" />}>
            ...
            <div className="flex min-h-[320px] items-center justify-center rounded-lg border border-dashed border-border">
              <div className="text-center">
                <div className="mx-auto mb-4 flex size-16 items-center justify-center rounded-full bg-primary/5">
                  <FileCheck className="size-8 text-primary/30" />
                </div>
                <h3 className="text-sm font-semibold text-foreground">
                  Aucun avoir
                </h3>
                <p className="mt-1 max-w-sm text-sm text-muted-foreground">
                  Émettez un avoir pour annuler ou corriger une facture existante et créditer votre client.
                </p>
                <Button className="mt-4 gap-2" size="sm" nativeButton={false} render={<Link href="/avoirs/new" />}>
                  <Plus className="size-4" />
                  Créer un avoir
                </Button>
              </div>
